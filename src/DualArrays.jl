module DualArrays
export DualVector, DualArray, DualMatrix
export dropzeros

import Base: +, ==, getindex, size, broadcast, axes, broadcasted, show, sum,
             vcat, convert, *, -, ^, /, ndims, hcat, vec, promote_rule, zero,
             reshape
using LinearAlgebra, ArrayLayouts, BandedMatrices, FillArrays, ComponentArrays
...
function DualMatrix(x::AbstractMatrix{Dual{T}}) where {T}
    val = zeros(T, size(x)...)
    jac = zeros(T, size(x)..., size(x[1,1].partials)...)
    n, m = size(x)
    for i = 1:n, j = 1:m
        val[i,j] = x[i,j].value
        jac[i,j,:,:] = x[i,j].partials
    end
    DualMatrix(val, jac)
end
...
function *(x::DualMatrix, y::DualVector)
    val = x.value * y.value
    t = promote_type(eltype(x.value), eltype(y.value))
    jac = zeros(t, length(val), size(y.jacobian, 2))
    n, m = size(jac, 1), size(x.value, 2)
    for i = 1:n
        new_row = zeros(size(jac, 2))
        for j = 1:m
            new_row += x.value[i,j] * y.jacobian[j,:] + y.value[j] * vec(x.jacobian[i, j, :, :])
        end
        jac[i,:] = new_row
    end
    DualVector(val, jac)
...
function sum(x::DualArray)
    #Drop redundant leading dimensions of 1 in jacobian
    idx = ntuple(i -> i > ndims(x) ? Colon() : 1, 2*ndims(x))
    Dual(sum(x.value), sum(x.jacobian; dims=1:ndims(x))[idx...])
end
...
function reshape(x::DualVector,dims::Vararg{Int})
    ...
    jac = reshape(x.jacobian, dims..., size(x.jacobian, 2), 1)
    DualMatrix(val, jac)
end

function vec(x::DualArray)
    val = vec(x.value)
    jac = reshape(x.jacobian, length(val), prod(size(x.jacobian)[ndims(x)+1:end]))
    DualVector(val, jac)
end
